...
Sub Bumper1_Hit
    vpmTimer.PulseSw 31
    PlaySound SoundFX("fx_BumperLeft", DOFContactors), 0, 1.5, -0.8, -0.5, 1.0, 0, 0
...
Sub Bumper2_Hit
    vpmTimer.PulseSw 32
    PlaySound SoundFX("fx_BumperRight", DOFContactors), 0, 1.5, -0.6, -0.5, 1.0, 0, 0
...
Sub Bumper3_Hit
    vpmTimer.PulseSw 33
    PlaySound SoundFX("fx_BumperMiddle", DOFContactors), 0, 1.5, -0.7, -0.5, 1.0, 0, 0
...
Sub SolLFlipper(Enabled)
    If Enabled Then
        PlaySound SoundFX("fx_flipperUp",DOFContactors), 0, 1, -1, 0, 0, false, false, 1
        LeftFlipper.RotateToEnd
    Else
		PlaySound SoundFX("fx_flipperdown",DOFContactors), 0, 1, -1, 0, 0, false, false, 1
...
Sub RollingSoundUpdate()
    ...
    For b = 0 to UBound(BOT)
        If BallVel(BOT(b) ) > 1 AND BOT(b).z < 30 Then
            rolling(b) = TRUE
            PlaySound("fx_ballrolling" & b), -1, Vol(BOT(b) ) * 2.0, Pan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0
        Else
